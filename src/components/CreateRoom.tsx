...
export default function CreateRoom() {
  ...
  const navigate = useGameStore.getState;
      ...
      <div className="max-w-xl w-full space-y-6">
        ...
        <div className="bg-gray-900/50 p-4 rounded-lg border border-red-500/20">
          ...
          <input
            ...
            onChange={(e) => setSettings({ ...settings, cardsPerPlayer: parseInt(e.target.value) })}
            className="w-full accent-red-500"
          />
        </div>
        ...
        <div className="bg-gray-900/50 p-4 rounded-lg border border-red-500/20">
          ...
          <input
            ...
            min="3"
            max="10"
            value={settings.survivalDays}
            onChange={(e) => setSettings({ ...settings, survivalDays: parseInt(e.target.value) })}
            className="w-full accent-red-500"
          />
        </div>
        ...
        <div className="bg-gray-900/50 p-4 rounded-lg border border-red-500/20">
          ...
          <input
            ...
            min="1"
            max="5"
            value={settings.heartsCount}
            onChange={(e) => setSettings({ ...settings, heartsCount: parseInt(e.target.value) })}
            className="w-full accent-red-500"
          />
        </div>
        ...
        <div className="bg-gray-900/50 p-4 rounded-lg border border-red-500/20">
          ...
          >
            ...
            <option value="madness">Безумие</option>
          </select>
        </div>
        ...
        <div className="bg-gray-900/50 p-4 rounded-lg border border-red-500/20 space-y-3">
          ...
          <label className="flex items-center justify-between text-gray-300">
            <span>Включить разведку</span>
            <input
              type="checkbox"
              checked={settings.scoutEnabled}
              onChange={(e) => setSettings({ ...settings, scoutEnabled: e.target.checked })}
              className="w-5 h-5 accent-red-500"
            />
          </label>
          ...
          <label className="flex items-center justify-between text-gray-300">
            ...
            <input
              ...
              onChange={(e) => setSettings({ ...settings, eventsEnabled: e.target.checked })}
              className="w-5 h-5 accent-red-500"
            />
          </label>
          
          <label className="flex items-center justify-between text-gray-300">
            <span>Принудительное голосование</span>
            <input
              type="checkbox"
              checked={settings.forcedVotingEnabled}
              onChange={(e) => setSettings({ ...settings, forcedVotingEnabled: e.target.checked })}
              className="w-5 h-5 accent-red-500"
            />
          </label>
          
          <label className="flex items-center justify-between text-gray-300">
            <span>Таймер хода</span>
            <input
              type="checkbox"
